...
export default function NavBar(){
   ...
   const {account, user} = useContext(DashContext)
    ...
    {menu.map((item)=>{
        return <Link to={item.path} key={item.id} className={`${location.pathname === item.path || location.pathname + location.search === item.path ? 'text-orange-700':null} hover:scale-90 hover:text-orange-700`}>{item.title}</Link>
    })}
    </div>
    ...
    <p className="text-zinc-300 font-semibold">{account.type === 'Creator' ? account.creatorname : account.type === 'Brand' ? account.company :  <span>{user.firstname} {user.lastname}</span>}</p>
