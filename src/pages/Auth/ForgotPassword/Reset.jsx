...
export default function ResetPassword(){
    ...
    <form onSubmit={handleSubmit} className="w-full flex flex-col items-center space-y-4 pt-4 px-4 2xl:px-0 xl:px-0 lg:px-0 md:px-0">
     <BasicInput name="password" onChange={handleChange}  show={()=>setVisible(!visible)} type={visible ? "text" : "password"} required placeholder="New Password" start="bi bi-search text-white" end={`${visible ? 'bi bi-eye' :'bi bi-eye-slash' }  text-white`}/>
    <BasicInput name="confirmpassword" onChange={handleChange}  show={()=>setVisible(!visible)} type={visible ? "text" : "password"} required placeholder="Confirm Password" start="bi bi-search text-white" end={`${visible ? 'bi bi-eye' :'bi bi-eye-slash' }  text-white`}/>
     <BasicButton title="Reset" custom="w-full 2xl:w-1/2 xl:w-1/3 lg:w-1/3 md:w-1/2 sm:w-2/3 text-lg"/>
    </form>
    </div>
    </AuthWrapper>
}