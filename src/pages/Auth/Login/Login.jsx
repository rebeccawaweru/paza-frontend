...
import { usersLoginPost } from "../../../api/client";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await usersLoginPost("/login", values);
      ...
      <div className="flex flex-col text-center items-center justify-center space-y-2 mt-8">
        ...
        >
          <BasicInput
            ...
            start="bi bi-envelope text-white"
          />
          <BasicInput
            name="password"
            onChange={handleChange}
          ...
          <Link
            to="/forgotpassword"
            className="w-full 2xl:w-1/2 xl:w-1/3 lg:w-1/3 md:w-1/2 sm:w-2/3 text-sm text-left text-zinc-600 cursor-pointer font-bold"
          >
            Forgot password?
          </Link>
          <BasicButton
