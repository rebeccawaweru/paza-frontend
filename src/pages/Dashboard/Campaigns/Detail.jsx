...
export default function CampaignDetail() {
  ...
  const permission =
    campaign.createdby ===
    (account.creatorname || account.company || user.email);
  ...
  const percentage =
    (miles.length > 0 &&
      (miles.filter((item) => item.status === "Completed").length /
        miles.length) *
        100) ||
    0;
  ...
  const handleSave = async () => {
    try {
      const response = await campaignsPut(`/${id}`, { milestones: miles });
      if (response.data) {
        ...
        <div className="grid grid-cols-3 gap-2 -mt-8 mx-2">
          <div className="col-span-2 p-2 space-y-4 grey">
            ...
            <div className="flex space-x-8 text-sm text-zinc-300 font-bold">
              <p
                ...
                className={`${
                  tab === "Description" && "text-orange-700"
                } cursor-pointer`}
              >
                Description
              </p>
            ...
            {tab === "Description" && (
              <div>
                <p className="text-sm leading-loose tracking-wide text-zinc-300">
                  {campaign.description}
                </p>
                <div className="text-sm mt-2">
                  <p className="underline tracking-wider leading-loose">
                    Objectives
                  </p>
                  {campaign.goals &&
            ...
            {tab === "Tasks" && (
              <div className="bg-black p-4">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <BasicButton
                    title="+ New"
                    handleClick={() => navigate(`/newtask?campaign=${id}`)}
                  />
                </div>
                {data.length === 0 ? (
                  <p className="text-zinc-400 text-sm my-4">
                    <i className="bi bi-folder2-open mx-2"></i> No tasks
                  </p>
                ) : (
          ...
          <div className="col-span-1 space-y-8 text-sm text-zinc-300 px-2 pt-12">
            <div className="flex space-x-1 font-semibold text-white items-center">
              <p>Completion:</p>
              <LinearProgress
                sx={{ height: 5, width: "50%", borderRadius: 10 }}
                color="success"
                variant="determinate"
                value={percentage}
              />
            ...
            <div className="flex justify-between font-bold cursor-pointer leading-loose">
              <p
                className={`${
                  tab2 === "Milestones" && "border-b-2 border-orange-700"
                }`}
              >
                Milestones
              </p>
              <p
                className={`${
                  tab2 === "Teams" && "border-b-2 border-orange-700"
                }`}
              >
                Teams
              </p>
              <p
                className={`${
                  tab2 === "Comments" && "border-b-2 border-orange-700"
                }`}
              >
                Comments
              </p>
                ...
                {!addMil && permission && (
                  <div className="flex items-center flex-col 2xl:flex-row xl:flex-row lg:flex-row md:flex-row sm:flex-row justify-between">
                    <button
                      ...
                      className="text-zinc-300 font-semibold cursor-pointer hover:text-orange-700 hover:scale-90"
                    >
                      {" "}
                      + Create
                    </button>
                    <button
                      onClick={handleSave}
                      className="text-orange-700 font-bold hover:scale-90"
                    >
                      <i className="bi bi-floppy mr-2"></i>Save Changes
                    </button>
                  </div>
                )}
                {addMil ? (
                  <form onSubmit={handleSave2} className="space-y-4">
                    <div>
                    ...
                    <div>
                      ...
                      <BasicSelect
                        ...
                        custom="w-full grey"
                      >
                        <MenuItem value="Major Milestone">
                          Major Milestone
                        </MenuItem>
                        <MenuItem value="Minor Milestone">
                          Minor Milestone
                        </MenuItem>
                      </BasicSelect>
                    </div>
                    ...
                    <div className="flex justify-end items-center space-x-4 cursor-pointer my-2">
                      <button
                        onClick={() => {
                          setAddMil(false);
                          if (
                            !Object.values(mile).some((value) => value === "")
                          ) {
                            setMiles((prev) => [...prev, mile]);
                          }
                        }}
                        className="text-zinc-300 border border-zinc-500 py-2 px-4"
                      >
                        {" "}
                        Cancel
                      </button>
                      <button
                        type="submit"
                        className="bg-orange-700 py-2 px-4 text-black rounded-md font-bold hover:bg-white hover:scale-90"
                      >
                        + Submit
                      </button>
                    </div>
                  ...
                  <div className="space-y-12">
                    {miles && miles.length > 0 ? (
                      miles.map((item, index) => {
                        return (
                          <div key={index} className="flex items-center">
                            <p className="font-bold text-white w-1/3">
                              {item.start}
                            </p>
                            <Milestone
                              status={item.status}
                            ...
                            {permission && (
                              <div className="inline-flex space-x-2 mx-2 cursor-pointer">
                                <i
                                  className="bi bi-pen-fill text-green-500"
                                  onClick={() => {
                                    setAddMil(true);
                                    setMile(item);
                                    setMiles((prev) =>
                                      prev.filter(
                                        (item) => item !== miles[index]
                                      )
                                    );
                                  }}
                                ></i>
                                <i
                                  className="bi bi-trash-fill text-red-500"
                                  onClick={() =>
                                    setMiles((prev) =>
                                      prev.filter(
                                        (item) => item !== miles[index]
                      ...
                      <p className="text-zinc-400">
                        <i className="bi bi-folder2-open mx-2"></i> No
                        milestones
                      </p>
                    )}
                  </div>
